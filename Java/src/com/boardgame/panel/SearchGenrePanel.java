...
public class SearchGenrePanel extends JPanel {
	...
	private ArrayList<Object[]>		dataGenreList, dataGameList;
	...
	private void getGenre() {
		new SQLCall(
				"{ call genre_pack.get_genre_info(?) }",
				cs -> {
					try {
						...
						if (!dataGenreList.isEmpty()) dataSltName = dataGenreList.stream().map(row -> row[1]).toArray(String[]::new);
	...
	private void getEenreBoardGameInfo(int genre_id) {
		new SQLCall(
				"{ call genre_pack.get_serch_genre_boardgame_info(?, ?) }",
				cs -> {
					try {
